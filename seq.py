...
class Sequencer(mido.ports.BaseOutput):
	...
	sequence: string
		a string of 'x','o' and '-' where each character stands for a step in
		the sequence. 'x' represents an onset, '-' represents a break where no
		sound is played and 'o' represents a tie. 'o's can only follow an onset
		'x' directly. So a sequence 'xoo--' is valid whereas a sequence 'oox--'
		is invalid.
